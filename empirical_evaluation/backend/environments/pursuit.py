...
def create(model_id, comms, world_size, name):
    ...
    id = f"{name}{'-comms' if comms else ''}-v{model_id}"
    teammate, task = POSSIBLE_MODELS[possible_models[model_id]]
    pomdp = try_load(lambda: PursuitPOMDP(id, task, teammate, 0.20, comms, world_size), id)
    return pomdp
...
class PursuitPOMDP(PartiallyObservableMarkovDecisionProcess):
    ...
    def __init__(self,
        ...
        observation_probabilities = self._generate_observation_probabilities(states, pomdp_action_meanings, observations, max_view_distance)
    ...
    def  _generate_observation_probabilities(self, states, action_meanings, observations, noise):
        ...
        def possible_observations(action, next_state):
            if tuple(next_state) == tuple(ABSORBENT):
                ...
                if action == "locate":
                    teammate_direction = tuple(np.sign((dx1, dy1)))
                    teammate_possibilities = {
                        cell_mappings[teammate_direction]: 1 - noise,
                        4: noise,
                    } if teammate != (0, 0) else {
                        4: 1.0
                    }
                else:
                    teammate_possibilities = {
